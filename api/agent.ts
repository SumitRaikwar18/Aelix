...
// Environment variables (loaded via Vercel)
...
class SetWalletTool extends StructuredTool {
  schema = z.object({ privateKey: z.string().describe("The private key to set the wallet") });
  name = "setWallet";
  description = "Set the wallet using a private key";

  constructor(private tools: BlockchainTools) { super(); }
...
class GetWalletAddressTool extends StructuredTool {
  ...
  async _call() {
    ...
    return !wallet ? "❌ No wallet set. Please use `setWallet <privateKey>` first." : `👛 Current Wallet Address:\nAddress: <b>${wallet.address}</b>`;
  }
}
...
class SignMessageTool extends StructuredTool {
  schema = z.object({ message: z.string().describe("The message to sign") });
  name = "signMessage";
...
class GetTransactionHistoryTool extends StructuredTool {
  schema = z.object({ count: z.number().optional().default(5).describe("Number of transactions to fetch") });
...
class GetTokenPriceTool extends StructuredTool {
  schema = z.object({ token: z.string().describe("Token ticker (e.g., MONAD)") });
  name = "getTokenPrice";
...
class GetTrendingTokensTool extends StructuredTool {
  ...
  async _call() {
    try {
      const response = await axios.get<string>(`${MONAD_EXPLORER_URL}/tokens`, { headers: { "User-Agent": "Mozilla/5.0" } });
...
class GetFaucetTokensTool extends StructuredTool {
  schema = z.object({ address: z.string().describe("The wallet address to receive testnet MON tokens") });
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    log.error("Handler Error:", error.stack || error);
